...
| 参数 | S | M | L | XL | XXL |
|------|---|---|---|---|-----|
| **触觉控制回路** | 开环 | 50Hz闭环 | 200Hz闭环 | 500Hz闭环 | 1000Hz闭环 |
| **抓取力控制精度** | ±20% | ±10% | ±5% | ±2% | ±0.5% |
| **滑移检测响应** | — | <50ms | <20ms | <5ms | <1ms |
| **多指协调** | ✗ | 2指 | 3指 | 5指 | 柔性多指 |
| **接触估计延迟** | <100ms | <20ms | <10ms | <5ms | <1ms |
| **触觉编码器维度** | 32 | 64 | 128 | 256 | 512 |

### 8.2 力觉-控制集成规格

| 参数 | S | M | L | XL | XXL |
|------|---|---|---|---|-----|
| **力控模式** | 位置/力切换 | 力位混合 | 阻抗控制 | 自适应阻抗 | 主动顺应 |
| **力控带宽** | — | 3 Hz | 8 Hz | 15 Hz | 30 Hz |
| **力跟踪精度** | ±20% | ±10% | ±5% | ±2% | ±0.5% |
| **碰撞检测** | ✗ | ✓ | ✓ | ✓ | ✓ |
| **碰撞响应时间** | >100ms | <50ms | <20ms | <10ms | <5ms |
| **力觉编码器维度** | 16 | 32 | 64 | 128 | 256 |

### 8.3 IMU-控制集成规格

| 参数 | S | M | L | XL | XXL |
|------|---|---|---|---|-----|
| **姿态估计频率** | 100Hz | 200Hz | 500Hz | 1000Hz | 2000Hz |
| **姿态稳定时间** | <500ms | <200ms | <100ms | <50ms | <20ms |
| **姿态角精度** | ±5° | ±1° | ±0.5° | ±0.1° | ±0.01° |
| **角速度测量范围** | ±1000°/s | ±2000°/s | ±4000°/s | ±4000°/s | ±8000°/s |
| **加速度测量范围** | ±8g | ±16g | ±24g | ±40g | ±80g |
| **IMU编码器维度** | 32 | 64 | 128 | 256 | 512 |

### 8.4 传感器-执行器同步规格

| 参数 | S | M | L | XL | XXL |
|------|---|---|---|---|-----|
| **时钟同步精度** | ±10ms | ±1ms | ±500μs | ±100μs | ±10μs |
| **传感器-控制器延迟** | <100ms | <20ms | <10ms | <5ms | <1ms |
| **执行器响应时间** | <50ms | <10ms | <5ms | <1ms | <200μs |
| **闭环控制周期** | 50Hz | 100Hz | 200Hz | 500Hz | 1000Hz |
| **时间戳同步协议** | NTP | PTP | PTPv2 | PTPv2+White Rabbit | IEEE 1588 |

### 8.5 自主决策-控制集成规格

| 参数 | S | M | L | XL | XXL |
|------|---|---|---|---|-----|
| **规划频率** | 0.1 Hz | 1 Hz | 5 Hz | 10 Hz | 20 Hz |
| **重规划响应时间** | >5s | <1s | <500ms | <200ms | <50ms |
| **预测时域** | 0.5s | 2s | 5s | 10s | 30s |
| **MPC求解器** | OSQP | OSQP | OSQP+FORCE | Acados | Acados+FPGA |
| **学习模式** | 监督 | 模仿 | 强化 | 在线强化 | 元强化 |
| **安全验证** | 后验 | 后验+实时 | 前验+实时 | 前验+预测 | 形式化验证 |
...
| 参数 | S | M | L | XL | XXL |
|------|---|---|---|---|-----|
| **控制通信** | UART | CAN | EtherCAT | EtherCAT+TSN | EtherCAT+TSN+光纤 |
| **传感器通信** | USB 2.0 | USB 3.0 | Gige Vision | 10GbE | 40GbE |
| **上层通信** | WiFi | WiFi 5GHz | 有线以太网 | TSN以太网 | TSN+时间同步 |
| **延迟确定性** | 非确定 | 软实时 | 准确定 | 硬实时 | 硬实时+冗余 |

### 9.3 功耗五级规格

| 参数 | S | M | L | XL | XXL |
|------|---|---|---|---|-----|
| **传感器总功耗** | <2W | <5W | <15W | <30W | <60W |
| **计算总功耗** | <5W | <15W | <40W | <100W | <300W |
| **执行器功耗** | <50W | <200W | <500W | <1kW | <5kW |
| **系统总功耗** | <100W | <300W | <600W | <2kW | <10kW |
| **散热方式** | 被动 | 风冷 | 风冷+液冷 | 液冷 | 液冷+相变 |
| **能效比** | 0.5 ops/mW | 1 ops/mW | 2 ops/mW | 5 ops/mW | 10 ops/mW |
