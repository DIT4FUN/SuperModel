...
class GradeControllerConfig:
    ...
    def from_grade(cls, grade: AGVGrade) -> 'GradeControllerConfig':
        """从等级获取控制器配置"""
        spec = GRADE_CONTROL_SPECS[grade]
        # Filter to only fields defined in GradeControllerConfig
        known_fields = {
            'grade', 'max_velocity', 'max_angular_velocity', 'max_acceleration',
            'control_period', 'control_frequency', 'trajectory_mode', 'planning_algorithm',
            'safety_level', 'friction_compensation', 'feedforward', 'slip_detection',
            'adaptive_gain', 'velocity_profile', 'max_jerk', 'skill_timeout',
            'max_concurrent_skills', 'real_time_kernel', 'fault_tolerance', 'redundancy',
        }
        filtered = {k: v for k, v in spec.items() if k in known_fields}
        return cls(grade=grade, **filtered)
